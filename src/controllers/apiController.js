...
exports.getnewslist = (req, res) => {
  ...
  let sql = " SELECT id,title,add_time,left(zhaiyao,25) as zhaiyao,click,img_url as img_url FROM dt_article where img_url > '' and channel_id = 6 limit 0,10 "
...
exports.getgoods = (req, res) => {
  ...
  let sql = `SELECT a.id,a.title,a.add_time,left(a.zhaiyao,25) as zhaiyao,a.click,a.img_url as img_url,b.sell_price,b.market_price,b.stock_quantity FROM dt_article as a,dt_article_attribute_value b where a.id = b.article_id and a.channel_id = 7 limit ${skipcount},${pagesize} `
  console.log('获取图文资讯sql语句：============>', sql)
  req.db.driver.execQuery(sql, (err, datas) => {
    // 4.0 判断是否异常
    if (err) {
